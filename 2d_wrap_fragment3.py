...
g_inner = gmsh.model.occ.addCircle(0, 0, 0, R_skin, angle1=theta, angle2=theta + theta_e)
g_outer = gmsh.model.occ.addCircle(0, 0, 0, R_skin + Et, angle1=theta, angle2=theta + theta_e)
gl1 = gmsh.model.occ.addLine(gmsh.model.occ.addPoint((R_skin)*np.cos(theta), (R_skin)*np.sin(theta), 0),
                             gmsh.model.occ.addPoint((R_skin + Et)*np.cos(theta), (R_skin + Et)*np.sin(theta), 0))
gl2 = gmsh.model.occ.addLine(gmsh.model.occ.addPoint((R_skin)*np.cos(theta+theta_e), (R_skin)*np.sin(theta+theta_e), 0),
                             gmsh.model.occ.addPoint((R_skin + Et)*np.cos(theta+theta_e), (R_skin + Et)*np.sin(theta+theta_e), 0))
...
all_devices = [(2, e) for e in electrodes] + [(2, gsurf)]
...
electrode_parents_indices = range(len(tissue_inputs), len(tissue_inputs) + len(all_devices))
