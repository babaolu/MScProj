...
# Create a CG1 vector space for output (one value per mesh vertex)
V_out = fem.functionspace(msh, ("Lagrange", 1, (msh.geometry.dim,)))
u_out = fem.Function(V_out)

# Interpolate the solved displacement into the output space
# Some dolfinx versions accept u_out.interpolate(uh) directly:
try:
    u_out.interpolate(uh)
    print("Direct interpolation worked!!")
except Exception:
    # fallback: project using a simple L2 projection (assemble mass ad rhs)
    trial = ufl.TrialFunction(V_out)
    test = ufl.TestFunction(V_out)
    a_proj = ufl.inner(trial, test) * ufl.dx
    L_proj = ufl.inner(uh, test) * ufl.dx
    proj_prob = LinearProblem(a_proj, L_proj,
        petsc_options={"ksp_type": "preonly", "pc_type": "lu"})
    u_out = proj_prob.solve()
    print("Fell back to projection!!")

gdim = msh.geometry.dim
print(f"geometry dimesion is {gdim}, with shape of {msh.geometry.x.shape}")
num_points = msh.geometry.x.shape[0]
u_vals = u_out.x.array.reshape((-1, gdim))

if u_vals.shape[0] != num_points:
    raise RuntimeError(
        f"DOF/vertex mismatch: u_vis has {u_vals.shape[0]} rows, mesh has" 
        + f"{num_points} points"
        )
