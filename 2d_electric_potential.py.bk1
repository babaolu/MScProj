...
lambda_vals = np.zeros(contact_facets.shape, dtype=np.float64)
...
"""def compute_facet_gaps(u_vert_vals):
    ...
    normals = np.zeros((contact_facets.shape[0], 2), dtype=np.float64)
    coords = msh.geometry.x.copy()[:, :2]	# vertex coords (xy)
    for j, f in enumerate(contact_facets):
...
for it in range(maxiter):
    ...
    # If no active facets and not first interation we might be converged
    if n_active == 0 and it > 0:
        print("No active facets: ALM stopped at iteration", it)
        break

    # Build meshtags for active facets only: values 1 for active facets else 0
    active_values = np.zeros(contact_facets.shape, dtype=np.int32)
...
"""
    ...
    # Recompile gaps with u_new (more accurate)
    gaps_new, normals_new = compute_facet_gaps(u_vert_new_vals)

    # Update lambda_vals elementwise
    for j in range(contact_facets.shape[0]):
        update = lambda_vals[j] + rho * (gaps_new[j])
        # because gap = (x+u)·n - R ; penetration -> gap < 0, we want -gap
        # AL update lambda^{k+1} = max(0, lambda^k + rho*(g - u_new·n))
        # But our 'gaps' variable is (x+u)·n - R, so g - u·n = ??? To keep consistent we use:
        # Using gaps_new[j] computed as above (gap = (x+u)·n - R), we need (g - u·n) = ???
        # T simplify: update using penetration measure: lambda_new = max(0, lambda_old + rho * (-gaps_new[j])).
        lam_candidate = lambda_vals[j] + rho * (-gaps_new[j])
        if lam_candidate < 0:
            lam_candidate = 0.0
        lambda_vals[j] = lam_candidate

    # compute convergence measure
    diff = u_new.x.array - u_k.x.array
    norm_diff = np.linalg.norm(diff)
    print(f"ALM iter {it}: active={n_active}, ||du||={norm_diff:.0e}")
    u_k.x.array[:] = u_new.x.array[:]

    if norm_diff < tol:
        print("ALM converged on displacement.")
        break
