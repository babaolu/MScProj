...
a_bulk = ufl.inner(σ(u), ufl.grad(v)) * dx
...
r_vec = x - center
n = r_vec / ufl.sqrt(ufl.dot(r_vec, r_vec))

gap = ufl.dot(r_vec, n) - R_skin + ufl.dot(u, n)
gap_neg = ufl.min_value(gap, 0.0)

h = ufl.CellDiameter(msh)
gamma = fem.Constant(msh, default_scalar_type(10 * E_cnt))
...
problem = NonlinearProblem(R, u, bcs=bcs, J=J, petsc_options_prefix="contact_",
    petsc_options={ "snes_type": "newtonls", "snes_linesearch_type": "bt",
            "snes_rtol": 1e-8, "snes_atol": 1e-8, "ksp_type": "preonly",
            "pc_type": "lu"}
    )
problem.solve()
